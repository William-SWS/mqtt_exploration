...
## ADR-023 — Fonte semântica e contrato observado do dicionário MQTT_UAD

- **Data:** 2026-09-01
- **Status:** aceita
- **Decisão:** documentar todas as 67 colunas dos três CSVs em um único dicionário. Usar as Tabelas 1-3 do artigo MQTT_UAD como fonte semântica principal, completar campos omitidos com a referência oficial de filtros do Wireshark e usar os CSVs locais verificados como fonte autoritativa para schema, capitalização de rótulos, contagens, tipos observados, missingness e cardinalidade. Classificar cada coluna preliminarmente como `portable`, `derivable`, `ablation_only` ou `excluded`, mantendo identificadores, conteúdo cru, tempo absoluto e ordem fora da matriz principal.
